...
// This module pulls the FULL order history from the Wix store and
...
// option) so a very large store can be imported in a few
...
// in one run instead.
//
// This is the import half only — it deliberately does NOT push
// availability back to Wix. The push is the caller's job, so that
// every caller is forced to put it after the import (see the ordering
// note in lib/wix-push.ts). Callers today: app/api/wix-sync (the daily
// cron) and app/api/wix-push's POST (the "Push to Wix now" button).
...
export interface SyncResult {
  ok: boolean;
  /** HTTP status a route should answer with. 200 on success. */
  status: number;
  error?: string;
  totalFetched: number;
  imported: number;
  ...
  noSizeRecorded: NoSizeLine[];
  message: string;
}

function fail(status: number, error: string): SyncResult {
  return {
    ok: false, status, error,
    totalFetched: 0, imported: 0, fulfilled: 0, awaitingHandover: 0,
    unmatched: [], noSizeRecorded: [], message: error,
  };
}
...
export async function runWixSync(opts: { limit?: number } = {}): Promise<SyncResult> {
  if (!process.env.WIX_API_KEY || !process.env.WIX_SITE_ID) {
    return fail(400, 'Wix is not connected yet. Add WIX_API_KEY and WIX_SITE_ID.');
  }
  ...
  try {
    ...
    return fail(502, err instanceof Error ? err.message : 'Could not reach Wix');
  }
